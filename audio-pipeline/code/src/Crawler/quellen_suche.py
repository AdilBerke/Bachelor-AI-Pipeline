...
Diese Datei ist die zentrale Schaltstelle fuer Suche, Download, Import,
Statusanzeige und optionales Aufraeumen von Roh-Audios. Die frueheren
Einzelmodule sind hier eingebettet, damit im aktiven Projekt nur eine
Crawler-Datei gepflegt werden muss.

Wichtig fuer die Bachelorarbeit: Der Crawler gehoert zur Datensammlung und ist
bewusst von der MusicGen-Trainingspipeline getrennt. So kann klar beschrieben
werden, welche Schritte Daten beschaffen und welche Schritte das Modell
trainieren oder Audios erzeugen.
"""
...
except Exception:  # pragma: no cover - Fallback fuer isolierte Crawler-Nutzung
...
# Eingebettete Unterbefehle. Jeder Eintrag ist der Quellcode eines frueheren
# Einzelmoduls. Beim Start wird nur der angeforderte Unterbefehl ausgefuehrt.
MODULES = {
...
###############################################################################
# Abschnitt: Input-gesteuerte Top-10-Suche
###############################################################################


def _projektwurzel() -> Path:
    """Findet die Projektwurzel anhand der kompakten Hauptordnerstruktur."""
...
def _jetzt_utc() -> str:
    """Liefert einen stabilen UTC-Zeitstempel fuer Reports und Verlauf."""
    return datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")


def _slug(text: str) -> str:
    """Macht aus einer freien Eingabe einen kurzen Dateinamen-Bestandteil."""
...
def _video_id(value: str) -> str | None:
    """Extrahiert eine YouTube-ID aus URL, youtu.be-Link oder Roh-ID."""
    if not value:
        return None
...
def _youtube_url(video_id: str | None, fallback: str = "") -> str:
    """Erzeugt eine normale YouTube-URL, falls eine Video-ID vorhanden ist."""
    return f"https://www.youtube.com/watch?v={video_id}" if video_id else fallback


def _text_norm(text: str) -> str:
    """Normalisiert Text fuer einfache Aehnlichkeits- und Keyword-Pruefungen."""
...
def _woerter(text: str) -> set[str]:
    """Zerlegt Nutzereingaben in verwertbare Such- und Bewertungswoerter."""
...
def _genre_key_aus_input(text: str) -> str:
    """Ordnet freie Genre-Eingaben den internen LoFi-Genres zu."""
...
def _float_wert(value: Any) -> float | None:
    """Konvertiert optionale Zahlenfelder aus yt-dlp-Metadaten."""
    try:
        number = float(value)
    except (TypeError, ValueError):
        return None
...
def _dauer_minuten(seconds: Any) -> float | None:
    """Konvertiert Sekunden aus Suchmetadaten in Minuten."""
    try:
        value = float(seconds)
    except (TypeError, ValueError):
        return None
...
def _lese_jsonl(path: Path) -> list[dict[str, Any]]:
    """Liest JSONL defensiv, damit kaputte Zeilen Reports nicht blockieren."""
    rows: list[dict[str, Any]] = []
    if not path.exists():
        return rows
    with path.open("r", encoding="utf-8") as handle:
...
def _bekannte_videos(verlauf_path: Path, wurzel: Path) -> list[dict[str, Any]]:
    """Sammelt Quellen, die bereits lokal importiert oder heruntergeladen wurden.

    Alte Suchlisten aus ``verlauf.jsonl`` werden bewusst nicht mehr als
    Ausschlussgrund genutzt. Sonst verhindert eine fruehere Top5-Suche, dass ein
    guter Kandidat nach neuen Regeln erneut gewaehlt werden kann.
    """
...
def _ist_aehnlich_bekannt(kandidat: dict[str, Any], bekannte: list[dict[str, Any]]) -> tuple[bool, str]:
    """Prueft gleiche IDs/URLs und sehr aehnliche Titel im selben Kanal."""
...
def _suchanfragen(args: argparse.Namespace) -> list[str]:
    """Erzeugt mehrere Suchanfragen aus Genre, Stimmung, Dauer und Lizenz.

    Fuer jedes Genre wird derselbe breite Satz an Anfrage-Vorlagen auf den
    zentralen Genre-Kernbegriff (aus GENRE_KEYWORDS) angewendet - frueher
    hatte nur Guitar Lofi eine so grosse, spezifische Anfragenliste, alle
    anderen Genres nur 5 generische Anfragen. Ein kleiner, einseitiger
    Anfragenpool findet oft nicht die wirklich populaersten Videos, weil
    YouTubes Suche pro Anfrage nach Relevanz sortiert, nicht nach Aufrufen.
    """
    ...
    # Populaere Lofi-Mixe benennen sich selten als "royalty free". Wir suchen
    # deshalb breiter, behalten aber "instrumental"/"no vocals" als Leitplanke,
    # damit keine Sprach-, Rap- oder Vocal-Quellen in die Top-10 rutschen.
    for kern in kernbegriffe:
        queries.extend(
            [
...
def _yt_dlp_suche(queries: list[str], max_suchergebnisse: int) -> tuple[list[dict[str, Any]], str | None]:
    """Sucht per yt-dlp/ytsearch nach YouTube-Kandidaten, ohne Downloads zu starten."""
    try:
        import yt_dlp
    except Exception as exc:  # pragma: no cover - haengt von lokaler Umgebung ab
        return [], f"yt-dlp nicht verfuegbar: {type(exc).__name__}: {exc}"

    kandidaten: list[dict[str, Any]] = []
    if not queries:
        return kandidaten, None
    # max_suchergebnisse ist die Zielzahl PRO Anfrage, nicht ein Gesamtbudget,
    # das mit wachsender Anfragenzahl immer duenner wird - sonst schrumpft der
    # Suchpool jedes Mal, wenn neue Anfrage-Vorlagen dazukommen (z.B. 150/20
    # Anfragen ergaebe nur noch 8 Treffer pro Anfrage statt eines brauchbaren
    # Pools).
...
def _yt_dlp_detail_json(url: str) -> dict[str, Any] | None:
    """Laedt Metadaten in einem Subprozess, damit yt-dlp-Abstuerze isoliert bleiben."""
    if not url:
        return None
    command = [
        sys.executable,
...
def _erganze_yt_dlp_details(kandidaten: list[dict[str, Any]], max_details: int) -> list[dict[str, Any]]:
    """Laedt Aufrufe/Likes und bessere Titel, ohne Audio herunterzuladen."""
...
def _urls_aus_lokalen_reports(wurzel: Path) -> list[dict[str, Any]]:
    """Extrahiert vorhandene YouTube-Links aus lokalen Text-, CSV- und JSON-Reports."""
    suchorte = [
...
def _score_keywords(text: str, terms: set[str], extra: set[str] | None = None) -> float:
    """Bewertet, wie viele relevante Begriffe im Kandidatentext vorkommen."""
    if extra:
...
def _dauer_score(dauer_minuten: float | None, minimum: float, maximum: float) -> float:
    """Bewertet, ob die Videodauer in den gewuenschten Bereich passt."""
...
def _audio_eignung_score(text: str) -> float:
    """Schaetzt anhand des Textes, ob ein Video wahrscheinlich als Musikquelle taugt."""
    normalized = _text_norm(text)
...
def _log_score(value: Any, low: float, high: float, missing: float = 45.0) -> float:
    """Bewertet grosse Zahlen logarithmisch, damit Millionen-Views sichtbar zaehlen."""
...
def _popularitaet_score(kandidat: dict[str, Any]) -> float:
    """Kombiniert Aufrufe und Likes zu einem Popularitaetssignal."""
...
# Kernbegriffe je Genre: mindestens einer muss im Videotitel selbst stehen
# (nicht nur in Kanalname/Beschreibung/Tags) - direkt neben einem Lofi-Anker,
# also als zusammenhaengende Wortfolge wie "Jazz Lofi" oder "Lofi Jazz". Der
# Titel darf davor/danach beliebig weiteren Text haben, aber Kern- und
# Lofi-Wort muessen unmittelbar nebeneinanderstehen - sonst zaehlt der Titel
# nicht als Genre-Beleg.
...
# "lofi"/"lo-fi" (nach Normalisierung: "lofi"/"lo fi") ist der Lofi-Anker,
# den Kern- und Lofi-Wort direkt nebeneinander brauchen ("Jazz Lofi",
# "Lofi Guitar" etc.) - "Jazz"/"Guitar"/"Study"/"Dreamy" kommen naemlich
# massenhaft in voellig lofi-fremder Musik vor (z.B. "Relaxing Jazz Piano",
# "Guitar Cafe": generische Ambient-/Lounge-Kanaele ohne jeden Lofi-Bezug).
# "Chillhop" ist selbst schon ein etabliertes Lofi-Subgenre (das Label
# "Chillhop Music" hat den Begriff gepraegt) - anders als bei den anderen
# Genres braucht es hier kein zusaetzliches "Lofi" im Titel. Ohne diese
# Ausnahme wuerden z.B. "Chillhop Essentials", "Chill Study Beats" oder
# "Chillhop Yearmix" des Chillhop-Music-Kanals (Millionen Aufrufe) faelschlich
# ausgeschlossen, nur weil "Lofi" nicht direkt daneben steht.
_TITEL_SELBSTGENUEGSAM = {"chillhop_lofi"}


def _titel_zeigt_genre(titel: str, genre_key: str) -> bool:
    """Prueft, ob der Videotitel den Genre-Namen als zusammenhaengende Wortfolge zeigt."""
    ...
    # "lo fi" (zwei Woerter nach Normalisierung) zu "lofi" zusammenziehen,
    # damit die Nachbarschaftspruefung unten nur noch Einzelwoerter vergleicht.
    titel_norm = re.sub(r"\blo fi\b", "lofi", titel_norm)
    woerter = titel_norm.split()

    # Wortweise statt reiner Teilstring-Suche: das Kernwort darf am Anfang
    # eines Titelworts stehen, damit einfache Formen wie "Dreams" (statt
    # "Dream") oder "Beats" nicht an einem angehaengten "s" scheitern.
...
def _titel_grob_fuer_details_ok(kandidat: dict[str, Any], args: argparse.Namespace) -> bool:
    """Vorfilter vor teuren Detailabfragen.

    Bei Guitar Lofi sind YouTube-Titel oft freier formuliert als die finale
    Quellenregel. Deshalb lassen wir hier mehr Kandidaten zur Detailpruefung zu;
    die endgueltige LoFi-/Genre-Pruefung passiert danach weiterhin streng.
    """
...
def _lofi_quelle_ok(kandidat: dict[str, Any], args: argparse.Namespace) -> tuple[bool, str]:
    """Erzwingt echten LoFi- und Untergenre-Bezug vor jedem Ranking."""
    genre_key = _genre_key_aus_input(args.genre)
    titel = str(kandidat.get("titel") or "")
...
def _bewerte_kandidat(kandidat: dict[str, Any], args: argparse.Namespace, duplikat_score: float) -> dict[str, Any]:
    """Berechnet alle Teil-Scores und den gewichteten Gesamtwert."""
    text = " ".join(
...
def _begruendung(
    ...
    """Formuliert eine kurze deutsche Begruendung fuer den Score."""
    teile = []
    if genre >= 70:
...
def _dedupliziere(kandidaten: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Entfernt echte Duplikate aus der Kandidatenliste."""
    seen: set[str] = set()
    result: list[dict[str, Any]] = []
...
def _schreibe_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    """Schreibt eine CSV mit stabiler Spaltenreihenfolge."""
    path.parent.mkdir(parents=True, exist_ok=True)
    bevorzugt = [
        "rang",
        "status",
...
def _schreibe_json(path: Path, payload: Any) -> None:
    """Schreibt JSON lesbar fuer spaetere Bachelorarbeits-Auswertung."""
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def _verlauf_anhaengen(path: Path, rows: list[dict[str, Any]], run_name: str) -> None:
    """Dokumentiert die ausgewaehlten Top-10-Kandidaten als bekannte Videos."""
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a", encoding="utf-8") as handle:
        for row in rows:
            item = {
...
def _sortwert(row: dict[str, Any], key: str) -> float:
    """Liefert einen defensiven numerischen Sortierwert."""
...
def _beste_sortierung(row: dict[str, Any]) -> tuple[float, float, float, float, float]:
    """Sortiert gute Quellen nach Aufrufen (Top10 = meiste Aufrufe zuerst).

    Genre-, Lizenz- und Qualitaetskriterien sind bereits ein Hart-Filter
    (siehe _lofi_quelle_ok) - unter den so qualifizierten Kandidaten soll die
    Top10 dann rein nach Popularitaet (Aufrufe) geordnet sein, mit dem
    Gesamt-Score nur noch als Tie-Breaker bei gleicher Aufrufzahl.
    """
...
def _kandidaten_sortierung(row: dict[str, Any]) -> tuple[int, float, float, float, float, float]:
    """Setzt auswahlfaehige Kandidaten nach oben und sortiert sie best-first."""
...
def _tabelle_text(value: Any, width: int) -> str:
    """Kuerzt Text fuer die lesbare Top-10-Tabelle auf feste Breite."""
...
def _tabelle_zahl(value: Any, width: int) -> str:
    """Formatiert Zahlen rechtsbuendig mit deutschen Tausenderpunkten."""
...
def _tabelle_dauer(row: dict[str, Any]) -> str:
    """Formatiert die Videodauer fuer die lesbare Tabelle."""
...
def _schreibe_top10_tabelle(path: Path, rows: list[dict[str, Any]]) -> None:
    """Schreibt eine ausgerichtete Texttabelle fuer schnelles menschliches Lesen."""
...
def _parse_top10_args(argv: list[str]) -> argparse.Namespace:
    """Definiert die CLI fuer die input-gesteuerte Top-10-Suche."""
...
def _manuelle_urls(args: argparse.Namespace) -> list[dict[str, Any]]:
    """Uebernimmt manuell angegebene URLs in die Kandidatenliste."""
...
def _run_top10(argv: list[str]) -> int:
    """Fuehrt die Top-10-Suche aus und schreibt alle geforderten Reports."""
    ...
    if not args.ohne_detail_metadaten:
        # Detail-Metadaten (Aufrufe/Likes) sind pro Kandidat ein eigener
        # yt-dlp-Aufruf und damit der teuerste Schritt. Der Titel-Check
        # (_lofi_quelle_ok) braucht dagegen nur den bereits vom Flach-Suchlauf
        # gelieferten Titel und ist praktisch kostenlos. Deshalb zuerst grob
        # nach Titel vorfiltern und Details nur noch fuer die - meist sehr
        # wenigen - echten Kandidaten laden. So kann der Suchpool viel groesser
        # sein (mehr Ergebnisse pro Anfrage), ohne dass die Laufzeit explodiert.
...
def _parse_genres_args(argv: list[str]) -> argparse.Namespace:
    """Definiert die gewichtete Genre-Suche als Unterbefehl."""
    parser = argparse.ArgumentParser(
...
def _ausgewaehlte_genres(args: argparse.Namespace) -> list[dict[str, Any]]:
    """Filtert den Genre-Plan optional auf die vom Nutzer genannten Genres."""
...
def _schreibe_genre_plan(report_dir: Path, run_name: str, genres: list[dict[str, Any]]) -> tuple[Path, Path]:
    """Schreibt den Suchplan als CSV und JSON fuer die Bachelorarbeit."""
...
def _lese_top10_csv(path: Path, genre: dict[str, Any]) -> list[dict[str, str]]:
    """Liest eine Top-10-CSV und ergaenzt die Strategie-Spalten."""
    if not path.exists():
        return []
...
def _schreibe_top10_gesamt(report_dir: Path, run_name: str, rows: list[dict[str, str]]) -> Path:
    """Schreibt alle Genre-Top-10-Ergebnisse in eine gemeinsame CSV."""
...
def _run_genres(argv: list[str]) -> int:
    """Fuehrt die gewichtete Top-10-Suche fuer mehrere Lofi-Genres aus."""
...
def _parse_mp3_args(argv: list[str]) -> argparse.Namespace:
    """Definiert den MP3-Import als kompakten Crawler-Unterbefehl."""
    parser = argparse.ArgumentParser(
...
def _lese_url_datei_mp3(path: Path, fallback_genre: str) -> list[dict[str, str]]:
    """Liest MP3-Importlinks aus TXT oder CSV."""
    if not path.exists():
...
def _mp3_titel(entry: dict[str, str]) -> str:
    """Kombiniert Genre und Titel zu einem lesbaren lokalen Dateinamen."""
...
def _mp3_report_path() -> Path:
    """Erzeugt den Pfad fuer den MP3-Downloadreport."""
    wurzel = _projektwurzel()
...
def _run_mp3(argv: list[str]) -> int:
    """Laedt einzelne Links oder Linklisten mit der vorhandenen Importlogik."""
...
def _load_module(name: str) -> None:
    """Laedt ein eingebettetes Hilfsmodul einmalig in sys.modules."""
...
IMPORT_ZEITLIMIT_SEKUNDEN = 1800  # grosszuegig fuer mehrstuendige Lofi-Quellen (echte Downloads koennen mehrere Minuten dauern)


def _run_mit_zeitlimit(command: str, args: list[str], zeitlimit_sekunden: int) -> int:
    """Fuehrt einen Crawler-Unterbefehl in einem eigenen Kindprozess aus und
    bricht ihn nach einem festen Zeitlimit hart ab.

    In-Prozess-Ausfuehrung (ueber ``_run``/``exec``) liess sich nicht
    zuverlaessig per ``signal.alarm()`` unterbrechen: yt-dlp faengt intern
    pro Downloadstrategie bereits breite Exceptions ab, wodurch auch ein
    Zeitlimit-Signal dort verschluckt wurde, statt den Aufruf abzubrechen.
    Ein echter Kindprozess laesst sich dagegen garantiert per SIGKILL
    beenden, egal woran er intern haengt.
    """
...
def _run(command: str, args: list[str]) -> int:
    """Fuehrt einen Crawler-Unterbefehl mit seinen CLI-Argumenten aus."""
...
def _help() -> None:
    """Zeigt die verfuegbaren Crawler-Unterbefehle."""
...
def main() -> int:
    """CLI-Einstiegspunkt fuer alle Crawler-Befehle."""
